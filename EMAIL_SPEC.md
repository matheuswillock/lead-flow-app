...
- 2026-08-10 — **Estágio 9 concluído (D10, PR em aberto):** `commitDispatchTerminalState` envolvido com `withPrismaRetry` + `withDeadlockRetry`; `reconcileManualDispatchAfterError` persiste `totalSent` via fallback isolado quando o commit falha; caminho genérico de erro também preserva `totalSent` quando há EmailLog de sucesso. Script `scripts/reconcile-historical-dispatch-totals.ts` (dry-run padrão) para os 3 dispatches históricos do audit §8.3 — execução em produção só com autorização do owner.
