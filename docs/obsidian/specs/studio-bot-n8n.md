...
aliases:
  - Bethânia
  - Studio Bot N8N
  - Bot Bethânia Corretor Studio
canonical: specs/studio-bot-n8n.md
created: 2026-06-30
updated: 2026-06-30
status: implemented
---
...
> Fonte canônica no repositório: `specs/studio-bot-n8n.md`  
> Repo: `lead-flow-app` — branch `feature/studio-bot-*` (PRs incrementais)
...
| Webhooks | `app/api/webhooks/backoffice/studio-bot/` |
| Cron outbox | `GET /api/v1/notifications/cron/studio-bot-outbox` |
| Postman | pasta **Studio Bot (Bethânia)** |
...
- [x] Feature slug `studio-bot` — migration + seed
...
- [x] `POST /api/v1/bot/auth/request-code`
- [x] `POST /api/v1/bot/link/initiate` + `GET /api/v1/bot/link/status` + `DELETE /api/v1/bot/link`
- [x] `POST /api/v1/bot/auth/verify-code` + `GET /api/v1/bot/auth/status`
- [x] Webhook inbound + HMAC + idempotency
- [x] `BotPolicyService`
- [x] Postman + `lib/env/validation.ts`

### Fase 2 — N8N core flows (read)

- [x] Pasta `n8n/workflows/` com exports JSON versionados
- [x] `n8n/README.md` — import de workflows + URLs de webhook
- [x] Workflow roteador auth (`VINCULAR` vs primeiro contato)
- [x] Workflow verificação caminho A
- [x] Workflow verificação caminho B
- [x] UI: `BethaniaConnectionCard` em Minha conta → Conexões
- [x] Workflow menu principal
- [x] Workflows listar/buscar leads, agenda, tarefas
- [x] `GET /api/v1/bot/context`

### Fase 3 — N8N mutation flows (write)

- [x] Nota em lead
- [x] Upload anexo (mídia → storage → API)
- [x] Agendar / remarcar / cancelar reunião
- [x] Criar tarefa com confirmação
- [x] `ActivityType.studio_bot` + payload Bethânia

### Fase 4 — Notificações outbound

- [x] `BackofficeBotEventOutbox` + dispatcher
- [x] Hooks em schedule, task (assignment parcial v1)
- [x] Workflows N8N push com botões
- [x] Preferências de notificação (API)

### Fase 5 — Backoffice admin UI

- [x] Nav item Bethânia em `BackofficeSidebar` + `app/backoffice/(app)/studio-bot/`
- [x] `components/messaging/` extraído do inbox WhatsApp produto
- [x] Inbox read-only com adaptadores DTO
- [x] Form perfil Bethânia + sync provider
- [x] APIs conversations, messages, channel/profile, avatar, reconnect, sync-profile
- [x] Vinculações + revogar; log verificações auth
- [x] Postman backoffice bot endpoints

### Fase 6 — Hardening

- [x] Rate limits push (12/h) + helpers HSM/observabilidade
- [ ] Templates HSM pré-aprovados produção
- [x] Testes `BotPolicyService` + `governance:check`
- [ ] Testes E2E N8N automatizados (lista SPEC)
...
- [ ] Feature slug `studio-bot`: addon pago ou incluso no plano base
...
- [ ] Extrair `components/messaging/` na mesma PR da UI backoffice ou PR prévia de refactor?
- [ ] Bucket/storage para avatar da Bethânia: mesmo bucket de mídia WhatsApp ou dedicado `backoffice-bot`?
