...
export function CheckoutStep({ onBack }: CheckoutStepProps) {
  ...
  const handlePaymentConfirmed = () => {
    const emailParam = createdUserData?.email
      ? `&email=${encodeURIComponent(createdUserData.email)}`
      : "";
    toast.success("Pagamento confirmado!", {
      description: "Redirecionando para o login...",
    });
    router.push(`/sign-in?from=checkout&success=true${emailParam}`);
  };
  ...
  const handleBack = () => {
    ...
    if (createdUserData?.supabaseId) {
      ...
      router.push(`/sign-up?deleteUser=${createdUserData.supabaseId}`);
      return;
    }
    onBack();
  };
  ...
  const checkPaymentStatus = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/subscriptions/${subscriptionId}/notify-payment`);
      if (!response.ok) {
  ...
  const handleSubmit = async (data: SignUpCheckoutFormData) => {
    ...
    try {
      const payload: any = {
        ...
        postalCode: createdUserData.postalCode,
        address: createdUserData.address,
        addressNumber: createdUserData.addressNumber,
        complement: createdUserData.complement,
        province: createdUserData.neighborhood,
      ...
      const response = await fetch("/api/v1/subscriptions/create", {
        ...
        headers: {
          ...
          "x-supabase-user-id": createdUserData.supabaseId,
        },
      ...
      setSubscriptionId(dataResult.subscriptionId || null);
                      ...
                      : billingType === "PIX"
                        ...
                        : billingType === "BOLETO"
                          ? boletoData
                            ? "Validar pagamento"
                            : "Gerar boleto"
                          : `Pagar ${formatCurrency(PLAN_PRICE)}`}
                  </Button>
                  <Button
                    type="button"
                    variant="outline"
              ...
              <CardContent className="space-y-6">
                <div className="space-y-3">
                  <div className="flex items-center justify-between text-sm text-muted-foreground">
                    <span>Subtotal</span>
                    <span>{formatCurrency(PLAN_PRICE)}</span>
                  </div>
                ...
                <div className="border-t pt-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground">
                    <div className="space-y-1">
                      <p>Primeira cobranca</p>
                      <p className="text-foreground font-medium">{formattedFirstCharge}</p>
                    </div>
                    <div className="space-y-1">
                      <p>Ultima cobranca</p>
                      <p className="text-foreground font-medium">{formattedLastCharge}</p>
                    </div>
                  </div>
                </div>
