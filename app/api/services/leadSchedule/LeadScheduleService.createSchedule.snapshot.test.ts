...
const PREVIOUS_CLOSER_ID = "55555555-5555-4555-8555-555555555555"
...
const profileFindUniqueMock = mock(async ({ where }: { where: { id: string } }) => {
  ...
  if (where.id === PREVIOUS_CLOSER_ID) {
    return {
      ...
      fullName: "Beto Closer Anterior",
      googleConnection: previousCloserGoogleConnectionActive,
    }
  }
...
let existingScheduleFixture: Record<string, unknown> | null = null
const findLatestByLeadIdMock = mock(async () => existingScheduleFixture)
...
// --- Google Calendar ---
let upsertCalendarEventShouldFail = false
const upsertCalendarEventMock = mock(async (_options: { attendeeEmails: string[]; meetingFormatLabel?: string | null }) => {
  if (upsertCalendarEventShouldFail) {
    throw new Error("Falha simulada ao criar evento no Google Calendar")
  }
  return {
...
const cancelCalendarEventMock = mock(async (_options: { eventId: string }) => undefined)
...
// --- EmailService ---
let sendMeetingInviteEmailShouldFail = false
const sendMeetingContactNotificationEmailMock = mock(async () => ({ success: true }))
const sendMeetingInviteEmailMock = mock(async () =>
  sendMeetingInviteEmailShouldFail
    ? { success: false, error: "Falha simulada no Resend" }
    : { success: true, data: { id: "resend-invite-1" } }
)
...
describe("LeadScheduleService.createSchedule — T-13.4 (snapshot pré-refatoração A-E2/A-E3/A-E4)", () => {
  ...
  /**
   * Cenários extras (rede de segurança extra para A-E3, que mexe exatamente
   * nesses trechos: o despachante de convites por canal). Escritos antes da
   * extração de A-E3, junto com os 3 cenários originais de T-13.4.
   */

  it("online com Google: falha ao criar evento → retorna erro, não grava nada", async () => {
    googleConnectionActive = { refreshToken: "refresh-token", revokedAt: null }
    upsertCalendarEventShouldFail = true
    const { LeadScheduleService } = await import("./LeadScheduleService")
    const service = new LeadScheduleService()
    ...
    expect(output.errorMessages[0]).toContain("Google Calendar")
    expect(transactionMock).not.toHaveBeenCalled()
    expect(sendCloserScheduleNotificationEmailMock).not.toHaveBeenCalled()
  })

  it("online sem Google: falha no envio por Resend → retorna erro, não grava nada", async () => {
    googleConnectionActive = null
    sendMeetingInviteEmailShouldFail = true
    const { LeadScheduleService } = await import("./LeadScheduleService")
    const service = new LeadScheduleService()
    ...
    expect(output.isValid).toBe(false)
    expect(output.errorMessages[0]).toContain("Falha simulada no Resend")
    expect(transactionMock).not.toHaveBeenCalled()
  })

  it("telefone com Google conectado: cria também um evento pessoal (best-effort) no Calendar do closer", async () => {
    googleConnectionActive = { refreshToken: "refresh-token", revokedAt: null }
    const { LeadScheduleService } = await import("./LeadScheduleService")
    const service = new LeadScheduleService()

    const output = await service.createSchedule(baseParams({ meetingType: "whatsapp" }) as any)

    expect(output.isValid).toBe(true)
    expect(sendMeetingContactNotificationEmailMock).toHaveBeenCalledTimes(1)
    expect(upsertCalendarEventMock).toHaveBeenCalledTimes(1)
    const callArgs = upsertCalendarEventMock.mock.calls[0]?.[0] as any
    expect(callArgs.attendeeEmails).toEqual([])
    expect(callArgs.meetingFormatLabel).toBe("WhatsApp")
    // O status do convite continua "sent_resend" (o e-mail ao lead já
    // resolveu isso) — o evento pessoal é só um bônus best-effort.
    expect(output.result.inviteDispatch.status).toBe("sent_resend")
    expect(output.result.googleEventId).toBe("google-event-1")
  })

  it("troca de closer com evento existente no Google: cancela o evento do closer anterior", async () => {
    googleConnectionActive = { refreshToken: "refresh-token", revokedAt: null }
  ...
  it("troca de closer, mas closer anterior sem Google: não cancela (loga aviso), agendamento segue", async () => {
    googleConnectionActive = { refreshToken: "refresh-token", revokedAt: null }
    previousCloserGoogleConnectionActive = null
    existingScheduleFixture = {
      id: "schedule-existing-1",
