...
import { getCdpAccess } from "./getCdpAccess"

function makeRequest(headers: Record<string, string> = {}) {
  return new NextRequest("http://localhost/api/v1/cdp/profiles", { headers })
}
...
describe("getCdpAccess", () => {
  ...
  it("returns 401 when x-supabase-user-id is missing", async () => {
    ...
    const result = await getCdpAccess(makeRequest())
  ...
  it("returns 403 for operator role", async () => {
    ...
    const result = await getCdpAccess(
      makeRequest({ "x-supabase-user-id": "user-1", "x-team-id": "team-1" })
    )

    expect(result.status).toBe(403)
    expect(result.error?.errorMessages).toContain("Acesso negado à CDP")
  })
  ...
  it("returns access for manager with CDP feature slug", async () => {
  ...
  it("returns 403 for manager without CDP feature slug", async () => {
    getTeamAccessSpy.mockResolvedValue(makeTeamAccess(UserRole.manager))
    ...
    expect(result.status).toBe(403)
    expect(result.error?.errorMessages).toContain("Add-on CDP não está ativo para este time")
  })
})
