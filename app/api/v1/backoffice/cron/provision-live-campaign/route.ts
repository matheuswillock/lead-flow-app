...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const output = await withCronAudit(
      {
        cronKey: "provision-live-campaign",
        cronPath: "/api/v1/backoffice/cron/provision-live-campaign",
      },
      () => backofficeEmailCampaignUseCase.ensureUpcomingLiveCampaignExists(),
      {
    ...
    console.error("[BackofficeCronProvisionLiveCampaignRoute][GET]", error)
    return NextResponse.json(
      new Output(false, [], ["Erro interno no cron de provisionamento"], null),
      { status: 500 }
    )
  }
}
