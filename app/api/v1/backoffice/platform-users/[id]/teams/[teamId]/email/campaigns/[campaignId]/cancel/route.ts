...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const output = await backofficeStudioEmailUseCase.cancelCampaign(resolved.actor, all.campaignId)
    return studioEmailJson(output)
  } catch (error) {
    rethrowIfPrerenderInterrupted(error)
    console.error("[BackofficeStudioEmailCampaignCancelRoute][POST]", error)
