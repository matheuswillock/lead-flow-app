...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const output = await backofficeStudioEmailUseCase.submitTemplate(resolved.actor, all.templateId)
    return studioEmailJson(output)
  } catch (error) {
    rethrowIfPrerenderInterrupted(error)
    console.error("[BackofficeStudioEmailTemplateSubmitTemplateRoute][POST]", error)
