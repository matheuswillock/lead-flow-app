...
type RouteContext = { params: Promise<StudioEmailRouteParams & { logId: string }> }
...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const output = await backofficeStudioEmailUseCase.getLog(resolved.actor, all.logId)
    return studioEmailJson(output, 200, 404)
  } catch (error) {
    rethrowIfPrerenderInterrupted(error)
    console.error("[BackofficeStudioEmailLogByIdRoute][GET]", error)
