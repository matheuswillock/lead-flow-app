...
export async function POST(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    const output = await backofficeStudioEmailUseCase.archiveCampaign(resolved.actor, all.campaignId)
    return studioEmailJson(output)
  } catch (error) {
    rethrowIfPrerenderInterrupted(error)
    console.error("[BackofficeStudioEmailCampaignArchiveRoute][POST]", error)
