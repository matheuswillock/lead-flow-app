...
const bodySchema = z.object({
  ...
  packSha256: z.string().regex(/^[0-9a-f]{64}$/i),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    const output = await backofficeBotHostUseCase.syncHost(parsed.data, {
    ...
    console.error("[BackofficeBotHostSyncRoute][POST]", error);
