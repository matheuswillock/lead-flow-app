...
class SyncEmailContactToRadarUseCase {
  async execute(input: SyncEmailContactToRadarInput): Promise<Output> {
    try {
      ...
      const result = await radarService.syncFromEmail(scope, { emailContactId: input.emailContactId })
