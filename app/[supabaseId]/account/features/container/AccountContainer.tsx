 ...
 * Separado para poder usar o hook useAccount
 */
export function AccountContainer() {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    if (user?.supabaseId) {
      const result = await uploadProfileIcon(user.supabaseId, file);
      if (!result.isValid) {
  ...
  async function handleDeleteIcon() {
    if (user?.supabaseId) {
      const result = await deleteProfileIcon(user.supabaseId);
      if (result.isValid) {
  ...
  async function onSubmit(data: updateAccountFormData) {
    ...
    const result = await updateAccount(user.supabaseId, data);
