...
export function EmailUnsubscribeContainer() {
  const { loading, confirming, info, completed, error, handleConfirm } = useEmailUnsubscribeContext()
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-4">
            {loading ? (
              ...
              <Alert>
                <AlertDescription>
                  Pronto. Você não receberá mais e-mails de campanha deste time.
                </AlertDescription>
              </Alert>
            ) : (
              <>
                <p className="text-center text-sm text-muted-foreground">
                  Você ({info?.maskedEmail}) deixará de receber campanhas de e-mail de{" "}
                  <span className="font-medium text-foreground">{info?.teamName}</span>.
                </p>
                <Button
                  variant="destructive"
                  className="w-full"
