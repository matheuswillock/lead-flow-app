    ...
    return (
      <div
        ...
        className={cn(
          ...
          // Reserve space when expanded; shrink when collapsed.
          "md:w-[--sidebar-width]",
          "md:data-[collapsible=icon]:w-[--sidebar-width-icon]",
          "md:data-[collapsible=offcanvas]:w-0",
          // When collapsed, clip the inner content so it doesn't overlay.
          "overflow-hidden",
          className
        )}
        data-state={state}
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-7 w-7 max-lg:size-11", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
