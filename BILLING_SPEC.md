...
3. Implemente handlers para PAYMENT_REFUNDED, PAYMENT_PARTIALLY_REFUNDED, PAYMENT_CHARGEBACK_REQUESTED, PAYMENT_CHARGEBACK_DISPUTE, PAYMENT_AWAITING_CHARGEBACK_REVERSAL (todos confirmados como eventos reais na documentação oficial): localizar profile via subscription/customer, marcar subscriptionStatus 'past_due' nos dois primeiros e no chargeback requested/dispute; PAYMENT_AWAITING_CHARGEBACK_REVERSAL e o retorno a CONFIRMED/RECEIVED restauram o status anterior. Registrar em log estruturado + Sentry (SubscriptionChangeLog fica para o estágio correspondente).
...
4. Corrija a divergência de produção encontrada nesta auditoria: BackofficeProduct tem 2 variantes de CRM (R$150,00 fixo; R$120/89,90/79,90/100) cadastradas só no banco, fora de prisma/seed-backoffice-products.ts. Adicione essas variantes ao seed (ou confirme com o owner se são obsoletas e devem ser marcadas isActive:false) para que bun run db:migrate:reset:local reproduza o estado real.
...
**Não tocar:** valor cobrado de clientes já na assinatura legada (System A) — só a origem do número muda; webhook (Estágio 1 já resolvido); schema além do seed.

**Aceite:** `grep -rn "59\.9\|19\.9\|29\.9" app/` só encontra o helper de resolução; UI de assinatura exibe valores vindos da API; seed reproduz as variantes de produção. **Validação manual:** abrir `/[supabaseId]/subscription` e conferir preços; rodar `db:migrate:reset:local` e conferir que as variantes de CRM aparecem no Backoffice.
...
3. Na rota/UseCase de criação e edição de BackofficeAdhesion: se discountPercent > maxDiscountPercentWithoutApproval, a operação exige discountApprovedByProfileId preenchido por um profile com papel MANAGER/MASTER (getBackofficeAccess + verificação de papel) — sem isso, rejeitar com Output inválido explicando que precisa de aprovação.
...
**Aceite:** desconto aplicado ao total, nunca item a item; teto configurável em banco, não hardcoded; aprovação obrigatória acima do teto; tabela de preços central nunca é alterada por este fluxo. **Validação manual:** criar uma adesão de teste com 5% de desconto (abaixo do teto, sem aprovação) e outra com 30% (acima do teto, exigindo aprovação) e conferir os dois caminhos na UI.
...
1. Gatilho orientado a evento: no middleware ou no bootstrap autenticado (lib/bootstrap/getAuthenticatedLayoutBootstrapData.ts), quando o profile tem ProfileUserTypeAssignment com slug member_pro e accessExpiresAt já passado, dispare (fire-and-forget, sem bloquear a resposta) memberProBillingUseCase.syncUsageToSubscription(profileId, "member_pro_expiration") se ainda não foi sincronizado nas últimas N horas (evite disparar a cada request - use um lock/marker simples, ex.: comparar accessExpiresAt com um campo lastSyncedAt). O cron diário member-pro-expiration continua existindo como rede de segurança para contas que não fazem login logo após a expiração.
...
5. Testes: gatilho de evento dispara sync sem duplicar quando o cron já processou; validação prévia sinaliza cliente sem dados cadastrais na concessão; falha de sync aparece no Sentry (mock); e-mail usa a copy correta para os dois casos.
Rode bun run typecheck, bun run lint, bun run governance:check.
```
