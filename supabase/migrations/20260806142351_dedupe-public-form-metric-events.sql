-- Dedupe + normalize public form completion metric events inflated by non-idempotent submits.
-- 1) Keep earliest row per (publicationId, eventType, visitorSessionId)
-- 2) Keep earliest row globally per (eventType, visitorSessionId) so eventKey can be stable
-- 3) Normalize eventKey to visitorSessionId:eventType (matches app upsert contract)
