...
- Any net-new visual component **MUST** start with the `shadcn` skill workflow documented at `https://ui.shadcn.com/docs/skills` before implementation.
- Agents **MUST** prefer existing `shadcn/ui` components and compose them before introducing custom visual markup.
- When the `shadcn` workflow requires CLI commands in this repository, agents **MUST** use Bun-based commands (for example `bunx --bun shadcn@latest ...`).
...
- Use browser-native dialogs in web UI (`window.alert`, `window.confirm`, `window.prompt`, or global `alert/confirm/prompt`). Use shadcn `AlertDialog`/`Dialog` for confirmation flows and `sonner` for user feedback.
...
- In forms with required fields, the submit action **MUST** remain disabled until all required inputs pass validation (`formState.isValid` or equivalent).
...
7. Browser-native dialog usage in frontend source (`window.alert/confirm/prompt` or global `alert/confirm/prompt`) (blocked).
