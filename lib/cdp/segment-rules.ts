...
import { CRM_CLOSED_STATUSES, RECENT_CAMPAIGN_WINDOW_DAYS, type CdpSegmentSlug } from "@/lib/cdp/segment-config"

export type CdpSegmentEvent = {
  eventType: string
  occurredAt: Date
  metadata?: unknown
}

export type CdpSegmentProfileInput = {
  normalizedPrimaryEmail: string | null
  consents: { status: string }[]
  sourceLinks: { sourceType: string; sourceMetadata?: unknown }[]
  events: CdpSegmentEvent[]
...
export function profileMatchesCdpSegment(
  profile: CdpSegmentProfileInput,
