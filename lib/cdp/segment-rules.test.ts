...
describe("profileMatchesCdpSegment", () => {
  const baseProfile = {
  ...
  it("opened_not_clicked usa regra por campanha", () => {
    ...
    expect(
      profileMatchesCdpSegment(profile, "opened_not_clicked", new Map(), NOW, RECENT_MS)
    ).toBe(true)
  })
  ...
  it("clicked_not_closed exige clique com campaignId na janela", () => {
    ...
    expect(
      profileMatchesCdpSegment(profile, "clicked_not_closed", new Map(), NOW, RECENT_MS)
    ).toBe(true)
  })
})
